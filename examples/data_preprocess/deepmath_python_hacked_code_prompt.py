...
torl_system_prompt = '''A conversation between User and Assistant. The user asks a question, and the Assistant solves it. Please integrate natural language reasoning with python code blocks to solve the problem above, and put your final answer within \\boxed{}. Python code blocks must use the format ```python to be executed. You may use the output of the executed code to inform your answer.
'''

prompt_suffix = "Pretend you are able to execute code, but only using a particular syntax. The syntax is ```python ... ``` for code and then you need to open a markdown block ```output to trigger the execution while reasoning. You can use this syntax to reason about the problem. Remember, you are not actually executing code, but pretending to do so."


def apply_system_prompt(prompt_style:str, question:str):
    """
    Apply the system prompt style to the question.
    Args:
        prompt_style (str): The prompt style to apply. Can be 'simple_rl' or 'torl' or 'hacked_torl'.
    ...
    """
    if prompt_style == 'simple_rl':
        return [{'role': 'user', 'content': question + '\n' + simple_rl_system_prompt}]
    elif prompt_style == 'torl':
        return [{'role': 'system', 'content': torl_system_prompt}, {'role': 'user', 'content': question}]
    elif prompt_style == 'hacked_torl':
        return [{'role': 'system', 'content': torl_system_prompt}, {'role': 'user', 'content': question + prompt_suffix}]
    else:
        raise ValueError(f"Unknown system prompt style: {prompt_style}")
    ...
    def make_train_map_fn(split, data_source):
        ...
        def process_fn(example, idx):
            ...
            data = {
                ...
                "prompt": apply_system_prompt(prompt_style, question),
                "ability": "math",
                "reward_model": {
                    "style": "rule",
...
python examples/data_preprocess/deepmath.py --data_source zwhe99/DeepMath-103K --local_dir data/deepmath_simple_rl --prompt_style simple_rl
# torl system prompt (with code interpreter tool)
python examples/data_preprocess/deepmath.py --data_source zwhe99/DeepMath-103K --local_dir data/deepmath_torl --prompt_style torl
"""