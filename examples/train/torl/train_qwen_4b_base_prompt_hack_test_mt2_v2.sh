...
/data/${dataset_name}/math500_test.parquet,\
/data/${dataset_name}/aime24_test.parquet,\
/data/${dataset_name}/aime25_test.parquet]
...
ref_log_prob_max_token_len_per_gpu=$(( 4 * ( $max_prompt_length + $max_response_length ) ))
rollout_log_prob_max_token_len_per_gpu=$(( 4 * ( $max_prompt_length + $max_response_length ) ))
...
chat_template_override_jinja_path="/code/verl-tool/examples/train/torl/chat_template_override.jinja"

model_pretty_name=$(echo $model_name | tr '/' '_' | tr '[:upper:]' '[:lower:]')

if [ "$enable_agent" = "True" ]; then
    run_name="${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}-mt${max_turns}"
else
    run_name="${reward_manager}-${strategy}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}-mt${max_turns}"
fi
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    data.chat_template_override_jinja_path=$chat_template_override_jinja_path \
