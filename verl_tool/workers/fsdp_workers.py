from verl.workers.fsdp_workers import ActorRolloutRefWorker, AsyncActorRolloutRefWorker, Worker, DictConfig
...
class AgentAsyncActorRolloutRefWorkerMeta(type):
    def __new__(mcs, name, bases, attrs):
        if AsyncActorRolloutRefWorker in bases:
            # Create a dictionary to store super methods
            super_methods_record = {}
            
            # First pass: get methods defined in AgentAsyncActorRolloutRefWorker
            agent_methods = {method_name for method_name, method in attrs.items() 
                           if callable(method) and not method_name.startswith('__')}
            ...
            # Check which methods also exist in AsyncActorRolloutRefWorker
            for method_name in agent_methods:
                if hasattr(AsyncActorRolloutRefWorker, method_name):
                    super_methods_record[method_name] = AsyncActorRolloutRefWorker.__dict__.get(method_name)
            ...
            # Get the source code of AsyncActorRolloutRefWorker.__init__
            init_source = inspect.getsource(AsyncActorRolloutRefWorker.__init__)
            ...
            def combined_init(self, *args, **kwargs):
                ...
                # Execute the modified init body (skipping the def line and indentation)
                # This executes all the code from AsyncActorRolloutRefWorker.__init__ except super().__init__()
                exec(textwrap.dedent(modified_source.split('\n', 1)[1]), globals(), local_vars)
            ...
            # Copy other methods
            for method_name, method in AsyncActorRolloutRefWorker.__dict__.items():
                if not method_name.startswith('__') and method_name not in attrs:
                    attrs[method_name] = method
            ...
            new_bases = []
            for base in bases:
                if base is AsyncActorRolloutRefWorker:
                    if Worker not in new_bases:
                        new_bases.append(Worker)
                elif base not in new_bases:
                    new_bases.append(base)
...
class AgentAsyncActorRolloutRefWorker(Worker, AsyncActorRolloutRefWorker, metaclass=AgentAsyncActorRolloutRefWorkerMeta):
    def __agent_init__(self, config: DictConfig, role: str):
        self.config = config
        self.role = role
        self.agent_config = AgentActorConfig()
